...
?>
<div class="woocommerce_variation wc-metabox wooostify_variation_gallery_support closed">
	<h3>
		<a href="#" class="remove_variation delete" rel="<?php echo esc_attr( $variation_id ); ?>"><?php esc_html_e( 'Remove', 'woostify' ); ?></a>
		<div class="handlediv" aria-label="<?php esc_attr_e( 'Click to toggle', 'woostify' ); ?>"></div>
		<div class="tips sort" data-tip="<?php esc_attr_e( 'Drag and drop, or click to set admin variation order', 'woostify' ); ?>"></div>
		...
		foreach ( $product_object->get_attributes( 'edit' ) as $attribute ) {
			...
			<select name="attribute_<?php echo esc_attr( sanitize_title( $attribute->get_name() ) . "[{$loop}]" ); ?>">
				<option value="">
					...
					printf( esc_html__( 'Any %s&hellip;', 'woostify' ), wc_attribute_label( $attribute->get_name() ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					?>
				</option>
	...
	<div class="woocommerce_variable_attributes wc-metabox-content" style="display: none;">
		<div class="data">
			<p class="form-row form-row-first upload_image">
				<a href="#" class="upload_image_button tips <?php echo $variation_object->get_image_id( 'edit' ) ? 'remove' : ''; ?>" data-tip="<?php echo $variation_object->get_image_id( 'edit' ) ? esc_attr__( 'Remove this image', 'woostify' ) : esc_attr__( 'Upload an image', 'woostify' ); ?>" rel="<?php echo esc_attr( $variation_id ); ?>">
			...
			if ( wc_product_sku_enabled() ) {
				woocommerce_wp_text_input(
					array(
						...
						'label'         => '<abbr title="' . esc_attr__( 'Stock Keeping Unit', 'woostify' ) . '">' . esc_html__( 'SKU', 'woostify' ) . '</abbr>',
						'desc_tip'      => true,
						'description'   => __( 'SKU refers to a Stock-keeping unit, a unique identifier for each distinct product and service that can be purchased.', 'woostify' ),
						'wrapper_class' => 'form-row form-row-last',
					)
				);
			}
			?>
			<div class="row-product_variation_images">
				<?php if ( !class_exists( 'WC_Additional_Variation_Images' ) && !class_exists( 'Woo_Variation_Gallery' ) ) { ?>
					<div class="form-row form-row-full product_variation_images_container">
						<ul class="product_variation_images">
							<?php
							$product_variation_image_gallery = $variation_object->get_gallery_image_ids( 'edit' );
							$attachments                     = array_filter( $product_variation_image_gallery );
							$update_meta                     = false;
							$updated_variation_gallery_ids   = array();
							...
							if ( ! empty( $attachments ) ) {
								foreach ( $attachments as $attachment_id ) {
									...
									$updated_variation_gallery_ids[] = $attachment_id;
								}

								// need to update product meta to set new gallery ids.
								if ( $update_meta ) {
									update_post_meta( $post->ID, '_product_image_gallery', implode( ',', $updated_variation_gallery_ids ) );
								}
							}
							?>
						</ul>
						<div class="hide">
							<input type="hidden" class="product_image_variation_gallery" name="variation_gallery_image_ids[<?php echo esc_attr( $loop ); ?>]" value="<?php echo esc_attr( implode( ',', $updated_variation_gallery_ids ) ); ?>" />
						</div>
						<p class="add_product_variation_images hide-if-no-js">
							<a href="javascript:void(0);" class="add-product-variation-gallery" data-choose="<?php esc_attr_e( 'Add images to variation product gallery', 'woostify' ); ?>" data-update="<?php esc_attr_e( 'Add to gallery', 'woostify' ); ?>" data-delete="<?php esc_attr_e( 'Delete image', 'woostify' ); ?>" data-text="<?php esc_attr_e( 'Delete', 'woostify' ); ?>"><?php esc_html_e( 'Add product variation gallery images', 'woostify' ); ?></a>
						</p>
					</div>
				<?php } ?>
			</div>
			<p class="form-row form-row-full options">
				...
				<label class="tips" data-tip="<?php esc_attr_e( 'Enable this option if access is given to a downloadable file upon purchase of a product', 'woostify' ); ?>">
					<?php esc_html_e( 'Downloadable', 'woostify' ); ?>
				...
				<label class="tips" data-tip="<?php esc_attr_e( 'Enable this option if a product is not shipped or there is no shipping cost', 'woostify' ); ?>">
					<?php esc_html_e( 'Virtual', 'woostify' ); ?>
				...
				<?php if ( 'yes' === get_option( 'woocommerce_manage_stock' ) ) : ?>
					<label class="tips" data-tip="<?php esc_attr_e( 'Enable this option to enable stock management at variation level', 'woostify' ); ?>">
						<?php esc_html_e( 'Manage stock?', 'woostify' ); ?>
			...
			<div class="variable_pricing">
				...
				woocommerce_wp_text_input(
					array(
						...
						'placeholder'   => __( 'Variation price (required)', 'woostify' ),
					)
				);

				$label = sprintf(
					/* translators: %s: currency symbol */
					__( 'Sale price (%s)', 'woostify' ),
					get_woocommerce_currency_symbol()
				);
				...
				woocommerce_wp_text_input(
					array(
						...
						'data_type'     => 'price',
						'label'         => $label . ' <a href="#" class="sale_schedule">' . esc_html__( 'Schedule', 'woostify' ) . '</a><a href="#" class="cancel_sale_schedule hidden">' . esc_html__( 'Cancel schedule', 'woostify' ) . '</a>',
						'wrapper_class' => 'form-row form-row-last',
					)
				);
				...
				echo '<div class="form-field sale_price_dates_fields hidden">
					<p class="form-row form-row-first">
						<label>' . esc_html__( 'Sale start date', 'woostify' ) . '</label>
					...
					<p class="form-row form-row-last">
						<label>' . esc_html__( 'Sale end date', 'woostify' ) . '</label>
				...
				<div class="show_if_variation_manage_stock" style="display: none;">
					...
					woocommerce_wp_text_input(
						array(
							...
							'label'             => __( 'Stock quantity', 'woostify' ),
							'desc_tip'          => true,
							'description'       => __( "Enter a number to set stock quantity at the variation level. Use a variation's 'Manage stock?' check box above to enable/disable stock management at the variation level.", 'woostify' ),
							'type'              => 'number',
					...
					woocommerce_wp_select(
						array(
							...
							'label'         => __( 'Allow backorders?', 'woostify' ),
							'options'       => wc_get_product_backorder_options(),
							'desc_tip'      => true,
							'description'   => __( 'If managing stock, this controls whether or not backorders are allowed. If enabled, stock quantity can go below 0.', 'woostify' ),
							'wrapper_class' => 'form-row form-row-last',
						)
					);
					...
					$low_stock_placeholder = ( $product_object->get_manage_stock() && '' !== $product_object->get_low_stock_amount() )
						? sprintf(
							...
							esc_attr__( 'Parent product\'s threshold (%d)', 'woostify' ),
							esc_attr( $product_object->get_low_stock_amount() )
						)
						: sprintf(
							/* translators: %d: Amount of stock left */
							esc_attr__( 'Store-wide threshold (%d)', 'woostify' ),
					...
					woocommerce_wp_text_input(
						array(
							...
							'label'             => __( 'Low stock threshold', 'woostify' ),
							'desc_tip'          => true,
							'description'       => __( 'When variation stock reaches this amount you will be notified by email. The default value for all variations can be set in the product Inventory tab. The shop default value can be set in Settings > Products > Inventory.', 'woostify' ),
							'type'              => 'number',
							'custom_attributes' => array(
								'step' => 'any',
							),
			...
			<div>
				...
				woocommerce_wp_select(
					array(
						...
						'label'         => __( 'Stock status', 'woostify' ),
						'options'       => wc_get_product_stock_status_options(),
						'desc_tip'      => true,
						'description'   => __( 'Controls whether or not the product is listed as "in stock" or "out of stock" on the frontend.', 'woostify' ),
				...
				if ( wc_product_weight_enabled() ) {
					...
					woocommerce_wp_text_input(
						array(
							...
							'label'         => $label,
							'desc_tip'      => true,
							'description'   => __( 'Weight in decimal form', 'woostify' ),
							'type'          => 'text',
							'data_type'     => 'decimal',
					...
					<p class="form-field form-row dimensions_field hide_if_variation_virtual form-row-last">
						<label for="product_length">
							...
							printf(
								...
								esc_html__( 'Dimensions (L&times;W&times;H) (%s)', 'woostify' ),
								esc_html( get_option( 'woocommerce_dimension_unit' ) )
							);
							?>
						</label>
						<?php echo wp_kses_post( wc_help_tip( __( 'Length x width x height in decimal form', 'woostify' ) ) ); ?>
						<span class="wrap">
							<input id="product_length" placeholder="<?php echo $parent_length ? esc_attr( $parent_length ) : esc_attr__( 'Length', 'woostify' ); ?>" class="input-text wc_input_decimal" size="6" type="text" name="variable_length[<?php echo esc_attr( $loop ); ?>]" value="<?php echo esc_attr( wc_format_localized_decimal( $variation_object->get_length( 'edit' ) ) ); ?>" />
							<input placeholder="<?php echo $parent_width ? esc_attr( $parent_width ) : esc_attr__( 'Width', 'woostify' ); ?>" class="input-text wc_input_decimal" size="6" type="text" name="variable_width[<?php echo esc_attr( $loop ); ?>]" value="<?php echo esc_attr( wc_format_localized_decimal( $variation_object->get_width( 'edit' ) ) ); ?>" />
							<input placeholder="<?php echo $parent_height ? esc_attr( $parent_height ) : esc_attr__( 'Height', 'woostify' ); ?>" class="input-text wc_input_decimal last" size="6" type="text" name="variable_height[<?php echo esc_attr( $loop ); ?>]" value="<?php echo esc_attr( wc_format_localized_decimal( $variation_object->get_height( 'edit' ) ) ); ?>" />
						</span>
					</p>
					<?php
				}
			...
			<div>
				<p class="form-row hide_if_variation_virtual form-row-full">
					<label><?php esc_html_e( 'Shipping class', 'woostify' ); ?></label>
					<?php
					wp_dropdown_categories(
						array(
							'taxonomy'         => 'product_shipping_class',
							'hide_empty'       => 0,
							'show_option_none' => __( 'Same as parent', 'woostify' ),
				...
				if ( wc_tax_enabled() ) {
					woocommerce_wp_select(
						array(
							...
							'label'         => __( 'Tax class', 'woostify' ),
							'options'       => array( 'parent' => __( 'Same as parent', 'woostify' ) ) + wc_get_product_tax_class_options(),
							'desc_tip'      => 'true',
							'description'   => __( 'Choose a tax class for this product. Tax classes are used to apply different tax rates specific to certain types of product.', 'woostify' ),
			...
			<div>
				...
				woocommerce_wp_textarea_input(
					array(
						...
						'label'         => __( 'Description', 'woostify' ),
						'desc_tip'      => true,
						'description'   => __( 'Enter an optional description for this variation.', 'woostify' ),
						'wrapper_class' => 'form-row form-row-full',
					)
				);
				?>
			...
			<div class="show_if_variation_downloadable" style="display: none;">
				<div class="form-row form-row-full downloadable_files">
					<label><?php esc_html_e( 'Downloadable files', 'woostify' ); ?></label>
					<table class="widefat">
						<thead>
							<div>
								<th><?php esc_html_e( 'Name', 'woostify' ); ?> <?php echo wp_kses_post( wc_help_tip( __( 'This is the name of the download shown to the customer.', 'woostify' ) ) ); ?></th>
								<th colspan="2"><?php esc_html_e( 'File URL', 'woostify' ); ?> <?php echo wp_kses_post( wc_help_tip( __( 'This is the URL or absolute path to the file which customers will get access to. URLs entered here should already be encoded.', 'woostify' ) ) ); ?></th>
								<th>&nbsp;</th>
							</div>
						</thead>
						<tbody>
						...
						<tfoot>
							<div>
								<th colspan="1">
									...
									?>
									"><?php esc_html_e( 'Add file', 'woostify' ); ?></a>
								</th>
								<th colspan="3">
									<?php if ( $disabled_downloads_count ) : ?>
										...
										printf(
											...
											esc_html__( 'The indicated downloads have been disabled (invalid location or filetype&mdash;%1$slearn more%2$s).', 'woostify' ),
			...
			<div class="show_if_variation_downloadable" style="display: none;">
				...
				woocommerce_wp_text_input(
					array(
						...
						'label'             => __( 'Download limit', 'woostify' ),
						'placeholder'       => __( 'Unlimited', 'woostify' ),
						'description'       => __( 'Leave blank for unlimited re-downloads.', 'woostify' ),
						'type'              => 'number',
						'desc_tip'          => true,
						'custom_attributes' => array(
							'step' => '1',
				...
				woocommerce_wp_text_input(
					array(
						...
						'label'             => __( 'Download expiry', 'woostify' ),
						'placeholder'       => __( 'Never', 'woostify' ),
						'description'       => __( 'Enter the number of days before a download link expires, or leave blank.', 'woostify' ),
						'type'              => 'number',
						'desc_tip'          => true,
						'custom_attributes' => array(
							'step' => '1',
