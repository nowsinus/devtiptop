...
function dokan_daily_sales() {
    ...
    <form method="post" class="dokan-form-inline report-filter dokan-clearfix" action="" id="dokan-v-dashboard-reports">
        <div class="dokan-form-group">
            <label for="from"><?php _e( 'From:', 'dokan' ); ?></label> <input type="text" class="datepicker" name="start_date" id="from" readonly="readonly" value="<?php echo dokan_format_date( $start_date ); ?>" />
        </div>

        <input type="hidden" name="start_date_alt" id="from_alt" value="<?php echo esc_attr( $start_date->format( 'Y-m-d' ) ); ?>">
        <input type="hidden" name="end_date_alt" id="to_alt" value="<?php echo esc_attr( $end_date->format( 'Y-m-d' ) ); ?>">
        <?php wp_nonce_field( 'custom_range', 'dokan_report_filter_nonce' ); ?>

        <div class="dokan-form-group">
            <label for="to"><?php esc_html_e( 'To:', 'dokan' ); ?></label>
            <input type="text" name="end_date" id="to" class="datepicker" readonly="readonly" value="<?php echo dokan_format_date( $end_date ); ?>" />
