...
class AnnouncementController extends DokanRESTController {
    ...
    public function get_announcement( $request ) {
        ...
        if ( is_wp_error( $announcement ) || empty( $announcement ) ) {
            return new WP_Error( 'no_announcement_found', __( 'No announcement found', 'dokan' ), array( 'status' => 404 ) );
        }

        $data     = $this->prepare_response_for_object( $announcement, $request );
        $response = rest_ensure_response( $data );
     ...
     * @since 2.8.0
     *
     * @param \WP_Post $object_data
     * @param \WP_REST_Request $request
     *
     * @return WP_REST_Response
     */
    public function prepare_response_for_object( WP_Post $object_data, \WP_REST_Request $request ) {
        $data = array(
            'id'           => $object_data->ID,
            'title'        => $object_data->post_title,
            'content'      => $object_data->post_content,
            'status'       => $object_data->post_status,
            'created_at'   => mysql_to_rfc3339( $object_data->post_date ),
            'sender_type'  => get_post_meta( $object_data->ID, '_announcement_type', true ),
            'sender_ids'   => array(),
        );

        $sender_ids = get_post_meta( $object_data->ID, '_announcement_selected_user', true );
        ...
        $response = rest_ensure_response( $data );
        $response->add_links( $this->prepare_links( $object_data, $request ) );

        return apply_filters( 'dokan_rest_prepare_announcement_object', $response, $object_data, $request );
    }
